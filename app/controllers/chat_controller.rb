class ChatController < WebsocketRails::BaseController
    ...
    def client_connected
        ...
        broadcast_message :client_connected, FormatResponse.doFormat(true, nil, new_message)
    end
    
    def client_disconnected
        @@users_list = @@users_list.reject { |u| u[:socket_id] == client_id }
        new_message = {:users => @@users_list}
        broadcast_message :client_disconnected, FormatResponse.doFormat(true, nil, new_message)
    end
    ...
    def broadcast_message_from_client
        ...
        broadcast_message :broadcast_message, FormatResponse.doFormat(true, nil, new_message)
    end
    ...
    def private_message
        ...
        if user.nil?
            socket.send_message :private_message, FormatResponse.doFormat(false, nil, nil)
            send_message :private_message, FormatResponse.doFormat(false, nil, nil)
        else
            str_msg = message.sub user_name_code, ""
            
            resp = FormatResponse.doFormat(true, nil, {from: from, msg: str_msg}) 
