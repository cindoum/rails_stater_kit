class ChatController < WebsocketRails::BaseController
    ...
    def client_connected
        ...
        broadcast_message :client_connected, FormatResponse.doFormat(true, nil, new_message, false)
    end
    
    def client_disconnected
        @@users_list = @@users_list.reject { |u| u[:socket_id] == client_id }
        new_message = {:users => @@users_list}
        broadcast_message :client_disconnected, FormatResponse.doFormat(true, nil, new_message, false)
    end
    ...
    def broadcast_message_from_client
        ...
        broadcast_message :broadcast_message, FormatResponse.doFormat(true, nil, new_message, false)
    end
    ...
    def private_message
        ...
        if user.nil?
            socket.send_message :private_message, FormatResponse.doFormat(false, nil, nil, false)
            send_message :private_message, FormatResponse.doFormat(false, nil, nil, false)
        else
            str_msg = message.sub user_name_code, ""
            
            resp = FormatResponse.doFormat(true, nil, {from: from, msg: str_msg}, false) 
